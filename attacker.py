...
RSAPubKey = "<RSAKeyValue><Modulus></Modulus><Exponent>AQAB</Exponent></RSAKeyValue>"
auth = "WyI0MTg5MjUyNSIsImh0UkFVYXppQUpEMHBSMThDTzljdDdlcHBveGczbXVveVR5OTdSY2wiXQ=="
...
if result[0] == None or not Helpers.IsOnRightMachine(result[0], v=2):
    print("[x] The license does not work: {0}".format(result[1]))
    sys.exit()
else:
    print("[~] The license is valid!, waking up devils...")
    time.sleep(3)
    pass

# print credit message
...
~ Developed by Revenge of Devils Development Team,
  Use it at your own risk.
~ Join our Telegram for features updates and announcements.
~ Beta Version / Based on kali linux prompt style.
...
if prompt == "help":
    ...
    host = input(RED + f"\u250c\u2500\u2500({os.getlogin()}@{os.uname()[1]})-["+ YELLOW+"set-RHOST" + RED+"]\n\u2514> " + RESET)
    timeout = int(input(RED + f"\u250c\u2500\u2500({os.getlogin()}@{os.uname()[1]})-["+ YELLOW+"set-Timeout Value" + RED+"]\n\u2514> " + RESET))
    ...
    REPO_NAME = 'Drodexaron/Advanced-Network-Attacker'

    # The name of the Python module that contains the tool
    MODULE_NAME = 'attacker'

    # The current version of the tool
    CURRENT_VERSION = '0.5'

    # The URL of the raw file on GitHub that contains the latest version number
    VERSION_URL = f'https://raw.githubusercontent.com/{REPO_NAME}/main/{MODULE_NAME}/version.txt'

    # The URL of the raw file on GitHub that contains the tool code
    CODE_URL = f'https://raw.githubusercontent.com/{REPO_NAME}/main/{MODULE_NAME}/{MODULE_NAME}.py'

    def update_tool():
        """Check for updates to the tool and update if a new version is available."""
        ...
        if latest_version > CURRENT_VERSION:
            print(f'Updating {MODULE_NAME} from version {CURRENT_VERSION} to {latest_version}...')

            # Get the updated code from GitHub
            response = requests.get(CODE_URL)
            code = response.text

            # Write the updated code to a temporary file
            temp_file = f'{MODULE_NAME}_temp.py'
            with open(temp_file, 'w') as f:
                f.write(code)

            # Load the updated module
            updated_module = importlib.import_module(f'{MODULE_NAME}_temp')

            # Replace the current module with the updated module
            import sys
            sys.modules[MODULE_NAME] = updated_module

            print(f'{MODULE_NAME} updated to version {latest_version}!')
        else:
            print(f'{MODULE_NAME} is up to date (version {CURRENT_VERSION}).')
...
# get user input
try:
    target = input(RED + f"\u250c\u2500\u2500({os.getlogin()}@{os.uname()[1]})-["+ YELLOW+"set-RHOST" + RED+"]\n\u2514> " + RESET)
    attack_type = input(RED + f"\u250c\u2500\u2500({os.getlogin()}@{os.uname()[1]})-["+ YELLOW+"set-ATTACK-TYPE" + RED+"]\n\u2514> " + RESET).upper()
    port = int(input(RED + f"\u250c\u2500\u2500({os.getlogin()}@{os.uname()[1]})-["+ YELLOW+"set-RPORT" + RED+"]\n\u2514> " + RESET))
    threads = int(input(RED + f"\u250c\u2500\u2500({os.getlogin()}@{os.uname()[1]})-["+ YELLOW+"set-THREADS" + RED+"]\n\u2514> " + RESET))
    use_proxy = input(RED + f"\u250c\u2500\u2500({os.getlogin()}@{os.uname()[1]})-["+ YELLOW+"use-PROXY" + RED+"]\n\u2514> " + RESET).lower() == "yes"
    proxy_ip = ""
    proxy_port = 0
    if use_proxy:
        proxy_ip = input(RED + f"\u250c\u2500\u2500({os.getlogin()}@{os.uname()[1]})-["+ YELLOW+"set-PROXY-IP" + RED+"]\n\u2514> " + RESET)
        proxy_port = int(input(RED + f"\u250c\u2500\u2500({os.getlogin()}@{os.uname()[1]})-["+ YELLOW+"set-PROXY-PORT" + RED+"]\n\u2514> " + RESET))
except ValueError:
...
# initialize socket and random data
